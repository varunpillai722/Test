# -*- coding: utf-8 -*-
"""
Created on Tue Jul 19 10:21:51 2022
...
def write_nrms(a, b):
    ...
    #writer = pd.CSVlWriter('NRMS.csv')
    #nrms.to_csv(writer, index=False, header=None)
    #writer.save()
    print('--Done-- ')
    return
...
def main():
    ...
    IncompleteDatasetPath = "C:\\Users\\suraj\\Downloads\\Studies\\Data Mining\\Incomplete datasets\\Data 1"  # provide path for incompelet data
    ImputedDatasetPath = 'C:\\Users\\suraj\\Downloads\\Studies\\Data Mining'     # provide path to write imputed data excel sheet
    CompleteDatasetPath = "C:\\Users\\suraj\\Downloads\\Studies\\Data Mining\\Complete datasets\\Data_1.csv"   # provide path for complete data  
    NoOfCenters = 10  # No of clusters in data

    CompleteData = pd.read_csv(CompleteDatasetPath,  sep=',')
    ...
    for f in files:
        print('-----------Working on '+f+'-----------')
        os.chdir(IncompleteDatasetPath)
        IncompleteData = pd.read_csv(f, sep=',', header=None)
        ...
        #writer = pd.CSVWriter(f)
        #ImputedData.to_csv(writer, index=False, header=None)
        #writer.save()
